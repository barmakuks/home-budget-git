...
void SetUpDate(QDateEdit* minDate,
               QDateEdit* maxDate,
               const hb::chrono::DateInterval& interval,
               bool& stopper,
               Callback update_func)
{
    ...
    const QDate from = QDatefromNormalizedDate(interval.from);
    minDate->setDate(from);
    const QDate to = QDatefromNormalizedDate(interval.to);
    maxDate->setDate(to);
...
void SetPeriodComboBox(QComboBox* periodCB,
                       ...
                       Callback update_func)
{
    ...
    hb::chrono::DateInterval interval(NormalizeDate(dateFrom), NormalizeDate(dateTo));

    const auto period = GetDatePeriod(interval);
    ...
    , m_doc_dlg(NULL)
{
    ...
    ui->reportSubItemsTableView->horizontalHeader()->setSectionResizeMode(
    ...
    connect(
        ui->documentsTableView->selectionModel(),
...
void MainWindow::on_periodDocsCB_currentIndexChanged(int index)
{
    ...
    SetUpDate(ui->startDateDocsEdit, ui->endDateDocsEdit,
              GetDateInterval(static_cast<DatePeriod::Period>(index)), m_filterSetupInProgress,
...
void MainWindow::ApplyDocumentsFilter()
{
    ...
    hb::CurrencyId currencyId
        = m_currenciesDocModel.GetCurrencyItemId(ui->currencyDocsCB->currentIndex());

    QModelIndexList selection = ui->documentsTableView->selectionModel()->selectedRows();

    m_documentsModel.Reload(ui->startDateDocsEdit->date(), ui->endDateDocsEdit->date(), accountId,
                            currencyId, m_docTypeModel.GetSelected());

    m_paymentsModel.Reload(ui->startDateDocsEdit->date(), ui->endDateDocsEdit->date());
...
// class CurEx : public hb::ICurrencyRatesReceiver
//{
//    // ICurrencyExchangeRatesReceiver interface
// public:
//    virtual void OnCurrencyExchangeRatesReceived(const hb::Date& date,
//                                                 const hb::ExchangeRateTable& rates)
//    {
//        using namespace hb;
//        BalancePtr balance = Engine::GetInstance().GetBalance(date);
//        TotalBalancePtr total_balance = balance->GetTotalBalance();

//        for (Balance::const_iterator it = total_balance->begin(); it != total_balance->end();
//        ++it)
//        {
...
//            {
//                std::cout << " in UAH: "
//                          << hb::utils::FormatMoney(rates.at(980).at((*it)->Currency())
//                                                    * (*it)->Amount())
//                          << "uah";
//            }
...
void MainWindow::UpdateBalance()
{
    ...
    //    hb::CurrencyExchangeManager::RequestRates(hb::utils::NormalizeDate(ui->calendarWidget->selectedDate()),
    //    &currency);
}
...
void MainWindow::CalculateSelectedRows()
{
    ...
    typedef std::map<hb::CurrencyId, std::pair<hb::Money, hb::Money> > DayBalance;  // currency Id,
                                                                                    // <income,
                                                                                    // outcome>
    ...
    for (QModelIndexList::iterator it = indexes.begin(); it != indexes.end(); ++it)
    {
        ...
        if (doc)
        {
            if (doc->AmountTo().is_initialized() && !doc->AmountFrom().is_initialized())
            {
                ...
                {
                    day_balance.insert(
                        std::make_pair(amount.Currency(), std::make_pair(amount.Value(), 0)));
                }
                else
                {
                    DayBalance::mapped_type balance = day_balance[amount.Currency()];
                    day_balance[amount.Currency()]
                        = std::make_pair(balance.first + amount.Value(), balance.second);
                }
            }
            else if (!doc->AmountTo().is_initialized() && doc->AmountFrom().is_initialized())
            {
                ...
                {
                    day_balance.insert(
                        std::make_pair(amount.Currency(), std::make_pair(0, amount.Value())));
                }
                else
                {
                    DayBalance::mapped_type balance = day_balance[amount.Currency()];
    ...
    for (DayBalance::iterator it = day_balance.begin(); it != day_balance.end(); ++it)
    {
        ...
        balance_str << "  :" << std::setfill(' ') << std::setw(15)
                    << hb::qt_utils::FormatMoney(it->second.first) << " " << currency->Symbol()
                    << std::endl;
    }

    ui->summaLabel->setText(hb::qt_utils::Tr(balance_str.str()));
}

void MainWindow::CreateDocument(hb::DocumentType::Direction docType)
{
    using namespace hb;

    bool result = false;

    if (docType == hb::DocumentType::Direction::Movement)
    {
...
{
    SetPeriodComboBox(ui->periodDocsCB, ui->startDateDocsEdit->date(), ui->endDateDocsEdit->date(),
                      m_filterSetupInProgress, std::bind(&MainWindow::ApplyDocumentsFilter, this));
}
...
{
    CreateDocument(hb::DocumentType::Direction::Income);
}

void MainWindow::on_debitButton_clicked()
{
    CreateDocument(hb::DocumentType::Direction::Outcome);
}
...
void MainWindow::on_removeButton_clicked()
{
    ...
    if (indexes.size() == 1)
    {
        ...
        msgBox.setInformativeText(hb::qt_utils::Tr("Видалити цей документ?"));
...
void MainWindow::documentsTableView_selectionChanged(const QItemSelection& selected,
...
{
    CreateDocument(hb::DocumentType::Direction::Movement);
}
...
void MainWindow::MakeReport()
{
    ...
    const hb::CurrencyId currencyId
        = m_currenciesReportModel.GetCurrencyItemId(ui->currencyReportCB->currentIndex());
...
QGraphicsScene* MainWindow::CreateReportScene(const hb::ReportItem& report_item,
                                              hb::CurrencyId currency)
{
    using namespace hb;
...

    {
        ...
        if (cur_amount != amounts.end())
        {
            ...
            const std::string caption = report_item.DocTypeName() + ": "
                                        + hb::qt_utils::FormatMoney(amount + sub_amount) + " "
                                        + cur_amount->second.CurName();
            ...
            text->setPos(-text->boundingRect().width() / 2,
                         -radius - text->boundingRect().height() * 2.);
    ...
    for (auto it : report_item.SubItems())
    {
        ...
        {
            hb::Money amount
                = std::abs(cur_amount->second.Amount()) + std::abs(cur_amount->second.SubAmount());
    ...
    for (auto value : values)
    {
        ...
        str << value.first << " ";
        str << std::setw(2) << std::fixed << hb::qt_utils::FormatMoney(value.second)
            << currency_symbol;
        str << " (" << std::setw(2) << std::fixed << std::setprecision(1)
            << value.second * 100. / summa << "%)";
...
{
    SetUpDate(ui->startDateReportEdit, ui->endDateReportEdit,
              GetDateInterval(static_cast<hb::chrono::DatePeriod::Period>(index)),
              m_filterSetupInProgress, std::bind(&MainWindow::MakeReport, this));
}
...
{
    SetPeriodComboBox(ui->periodReportCB, date, ui->endDateReportEdit->date(),
                      m_filterSetupInProgress, std::bind(&MainWindow::MakeReport, this));
}

void MainWindow::on_endDateReportEdit_dateChanged(const QDate& date)
{
    SetPeriodComboBox(ui->periodReportCB, ui->startDateReportEdit->date(), date,
                      m_filterSetupInProgress, std::bind(&MainWindow::MakeReport, this));
}
